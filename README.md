...
- Designed a deterministic 4-agent pipeline: Foundry IQ → Sage Auditor → Chronos Scheduler → Surreal UI.
- Bound every decision to enterprise JSON policy before LLM generation.
- Added fault isolation with timeouts, bounded tokens, retries, and session-safe caching.
...
- Built a 3-pillar architecture: CNN acoustic analysis, LSTM fall/gait telemetry, and Dead Man’s Switch FSM.
- Used strict config validation, thread-safe event handling, and structured telemetry logging.
- Kept the design privacy-first with zero cloud dependency.
...
- Integrated Azure Translator, Web Speech API, and Tesseract.js into one pipeline.
- Added localStorage caching to reduce repeated API calls.
- Included auto language detection, swap, TTS, and floating assistant UI.
...
- Integrated Azure Document Intelligence for real-time parsing.
...
**Languages**  
`Python` `C++` `C` `JavaScript` `HTML5` `CSS3` `PHP`

**AI / ML / Cloud**  
`TensorFlow` `Azure` `OpenAI` `Streamlit`
...
- Solving DSA daily in **C++**.
- Focus areas: arrays, strings, recursion, linked lists, trees, sorting.
- Current topics: **Two Pointers, Sliding Window, Recursion**.
...
|---|---|---|
| Azure Fundamentals (AZ-900) | Microsoft | Cloud Computing |
| Threat Landscape 3.0 | Fortinet | Cybersecurity |
| Enterprise Design Thinking Practitioner | IBM SkillsBuild | Systems Thinking |
| Generative AI Studio | Google Cloud / Simplilearn | AI & LLMs |
| Professional Networking & Critical Thinking | HP LIFE | Professional Skills |
